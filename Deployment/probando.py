...
def objective_function(config, seed, budget):  # Eliminamos el argumento 'instance'
    ...
    for train_idx, val_idx in kfold.split(X_train_bal, y_train_bal):
        ...
        # Asegurar que la métrica AUC se usa en la compilación del modelo
        model.compile(
        ...
        # Callback de early stopping monitorizando AUC
        early_stop = tf.keras.callbacks.EarlyStopping(monitor='val_auc', patience=5, restore_best_weights=True, mode='max', verbose=0)
        ...
        # Guardar el mejor AUC alcanzado en validación
        best_auc = max(history.history['val_auc'])
        cv_aucs.append(best_auc)
    
    # Promedio de AUC en los folds
    return -np.mean(cv_aucs)  # Negativo porque optimizamos minimizando
