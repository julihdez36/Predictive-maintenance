"""
Probamos reducción de dimensionalidad por PCA
...
def preprocesar_datos(X_train_raw, y_train_raw, X_val_raw=None, aplicar_pca=False, n_componentes=0.95, scaler=None, pca=None):
    """
...
def objective_function(config, seed, budget):
    ...
    for train_idx, val_idx in kfold.split(X_train_full, y_train_full):
        ...
        # Preprocesar datos con QuantileTransformer, balanceo con SMOTETomek y PCA
        X_train_scaled, y_train_bal, X_val_scaled, scaler, pca = preprocesar_datos(
            X_train_raw, y_train_raw, X_val_raw, aplicar_pca=True, n_componentes=0.95
        )
